...
double calculate_mutation_rate(candidate_solution & antibody,
...
void calculate_affinity(std::vector<candidate_solution> & pop)
{
    ...
    double range = pop[pop.size() - 1].fitness - pop[0].fitness;
    if(range == 0.0)
    {
        for(auto & p : pop)
        {
    ...
    else
    {
        for(auto & p : pop)
        {
            p.affinity = 1.0 - pop[0].fitness / range;
        }
    }
}

void clone_and_hypermutate(std::vector<candidate_solution> & clones,
    std::vector<candidate_solution> & pop,
                           const size_t clone_factor)
{
    clones.clear();
    auto i_num_clones = num_clones(pop.size(), clone_factor);
    calculate_affinity(pop);
    for(auto & antibody : pop)
    {
        auto m_rate = calculate_mutation_rate(antibody);
        for(auto i = 0; i < i_num_clones; ++i)
        {
...
const size_t bits_per_param)
{
    ...
    new_pop.resize(pop.size() + rand_pop.size());
    std::copy(pop.begin(), pop.end(), new_pop.begin());
    std::copy(rand_pop.begin(), rand_pop.end(), new_pop.begin() + pop.size());
    std::sort(new_pop.begin(), new_pop.end(), [](const candidate_solution & a, const candidate_solution & b)
    {
        return a.fitness < b.fitness;
    });
    new_pop.resize(pop.size());
}
            ...
            const size_t bits_per_param = 16)
{
    ...
    for(size_t gen = 0; gen < max_gens; ++gen)
    {
        ...
        clone_and_hypermutate(clones, pop, clone_factor);
        evaluate(clones, search_space, bits_per_param);
        std::copy(clones.begin(), clones.end(), pop.begin());
        std::sort(pop.begin(), pop.end(), [](const candidate_solution & a, const candidate_solution & b)
        {
            return a.fitness < b.fitness;
        });
